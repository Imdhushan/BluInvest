        ...
        <card class="mb-3">
            <card-body>
                <div class="d-flex fw-bold small mb-3">
                    <span class="flex-grow-1">Yala,Third,Maha vs Income</span>
                    <card-expand-toggler />
                </div>
                <ApexCustomAreaChart
...
const third = ref([]);
const forth = ref([]);
const years = ref(['2020','2021','2022','2024'])
const getDataYears = async (selected_values,selected_area) => {
    ...
    localStorage.setItem("lineChart1",JSON.stringify(chartData.value));
...
onMounted(async () => {
    let existing_chart_data = localStorage.getItem('lineChart1');
