        ...
        <card class="mb-3">
            <card-body>
                <div class="d-flex fw-bold small mb-3">
                    <span class="flex-grow-1">Seasonal Incomes by {{ categoryKey }}</span>
                    <card-expand-toggler />
                </div>
                <ApexCustomBarChart
                    ...
                    :isStacked="true"
                    mainTitle="Yala,Third,Maha Incomes in District"
                />
            </card-body>
        </card>
...
const chartData = ref([]);
const attributes = ref([{name:'Yala Season',value:'Income_Y'},
    {name:'Third Season',value:'Income_T'},{name:'Maha Season',value:'Income_M'}])
const getDataYears = async (selected_values,selected_area,year) => {
    ...
    localStorage.setItem("barChart2",JSON.stringify(chartData.value));
}
...
onMounted(async () => {
    let existing_chart_data = localStorage.getItem('barChart2');
